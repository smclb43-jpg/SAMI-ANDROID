...
loginStationSelect.value = state.station;
...
function filteredJobs() {
  ...
  return state.jobs.filter(job => {
    const matchesStation = job.stage === state.station;
...
function updateSummary(rows) {
  ...
  document.getElementById("listTitle").textContent = `${state.station} Listesi`;
}
...
function render() {
  ...
  jobList.innerHTML = "";
  updateSummary(rows);
  currentStationSubtitle.textContent = `${state.station} İstasyonu`;
  ...
  rows.forEach(job => {
    ...
    node.querySelector(".job-stage").textContent = job.stage;
  ...
  if (apiBase) {
    try {
      const response = await fetch(`${apiBase}/api/stations/finish`, {
        ...
        body: JSON.stringify({ ids, stage: state.station })
      });
...
function openLogin() {
  loginStationSelect.value = state.station || "Kesim";
  loginApiUrlInput.value = localStorage.getItem(API_KEY) || "";
...
document.getElementById("loginContinueBtn").addEventListener("click", () => {
  state.station = loginStationSelect.value;
